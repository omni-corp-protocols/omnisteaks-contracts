...
import { Vault, StrategyMasterchefLP } from "../typechain";
...
async function main() {
  ...
  // Strategy
  const StrategyMasterchefLP = await hre.ethers.getContractFactory("StrategyMasterchefLP");
  const strategy: StrategyMasterchefLP = await StrategyMasterchefLP.deploy(
    config.poolId,
    config.wantToken,
    config.pool,
    vault.address,
  ...
  deployments["Constructors"][strategy.address] = StrategyMasterchefLP.interface.encodeDeploy([
    config.poolId,
    config.wantToken,
    config.pool,
    vault.address,
