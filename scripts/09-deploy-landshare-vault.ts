...
import { Vault, StrategyLandShareLP } from "../typechain";
...
async function main() {
  ...
  // Strategy
  const StrategyLandShareLP = await hre.ethers.getContractFactory("StrategyLandShareLP");
  const strategy: StrategyLandShareLP = await StrategyLandShareLP.deploy(
    config.wantToken,
    config.pool,
    vault.address,
    config.unirouter,
  ...
  deployments["Constructors"][strategy.address] = StrategyLandShareLP.interface.encodeDeploy([
    config.wantToken,
    config.pool,
    vault.address,
    config.unirouter,
