...
import { Vault, StrategyMasterchef } from "../typechain";
...
async function main() {
  ...
  // Strategy
  const StrategyMasterchef = await hre.ethers.getContractFactory("StrategyMasterchef");
  const strategy: StrategyMasterchef = await StrategyMasterchef.deploy(
    config.poolId,
    config.wantToken,
    config.pool,
    vault.address,
  ...
  deployments["Constructors"][strategy.address] = StrategyMasterchef.interface.encodeDeploy([
    config.poolId,
    config.wantToken,
    config.pool,
    vault.address,
