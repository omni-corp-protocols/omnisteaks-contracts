...
import { Vault, StrategyEllipsisBtcPool } from "../typechain";
...
async function main() {
  ...
  // Strategy
  const Strategy = await hre.ethers.getContractFactory("StrategyEllipsisBtcPool");
  const strategy: StrategyEllipsisBtcPool = await Strategy.deploy(
    vault.address,
    config.unirouter,
    deployer.address,
    deployer.address,
